...
class AmountParserNL(object):
    ...
    def __init__(self):
        ...
        UNIQUE_NUMBERS_MAP = {num2words(n, ordinal=True, lang=self.language).replace('een ', ''): n
                              for n in UNIQUE_NUMBERS}
        UNIQUE_NUMBERS_MAP.update(
            {num2words(n, lang=self.language).replace('een ', ''): n for n in UNIQUE_NUMBERS})
        UNIQUE_NUMBERS_MAP.update(
            {'één': 1,
             'een': 1,
             'eenhalf': 0.5,
        ...
        self.MAGNITUDE_MAP = {num2words(10 ** n, lang=self.language).replace('een ', ''): 10 ** n
        ...
        (?:
        (?:[\.\d][\d\.,]*\s+|\W|^)
        (?:(?:(?:in)?kwart|halve|{written_unique_numbers}|en)\s?)+(?:\W|$)|
        (?:[\.\d][\d\.,\s]*)
        )
        ...
        self.QUARTER_RE = re.compile(r'(?:\s*en\s+)?(een|twee|drie)\s*kwart')

        self.WRONG_FULLMATCH_RE = re.compile(r'\W*en\W*|\W+', re.IGNORECASE | re.MULTILINE | re.DOTALL)
...
get_amounts = AmountParserNL().parse


get_amount_annotations = AmountParserNL().parse_annotations
