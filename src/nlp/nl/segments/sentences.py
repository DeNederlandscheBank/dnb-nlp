...
"""Sentence segmentation for Dutch.

This module implements sentence segmentation in Dutch using simple machine learning classifiers.
...
from src.extract.en.en_language_tokens import EnLanguageTokens
from src.extract.nl.nl_language_tokens import NlLanguageTokens
...
SENTENCE_SEGMENTER_MODEL = joblib \
    .load(os.path.join(MODULE_PATH, "./sentence_segmenter.pickle"))  #
type: PunktSentenceTokenizer
extra_abbreviations = [a.rstrip('.') for a in NlLanguageTokens.abbreviations]
...
PRE_PROCESS_TEXT_REMOVE = re.compile(
    r'(?:^\s*\d+\s*$)'
    r'|(?:^\s*\<PAGINA\>\s*(\d+)?\s*(\n|$))'
    r'|(?:^\s*(^.+)?[Pp][Aa][Gg][Ii][Nn][Aa]\s+\d+\s?\/\s?\d+(.+)?$\s*(\n|$))'
    r'|(?:^\s+$)'
    r'|(?:^\s*i+\s*$)',
    re.MULTILINE
)
...
# are used in normalize_text for better splitting text on sentences PRETOKENIZE_REPLACEMENTS = [('“', '"'), ('”', '"')]
...
def get_sentence_span(text: str) -> Generator[Tuple[int, int, str], Any,
Any]:
    """
    ...
    for span in SENTENCE_SEGMENTER_MODEL.span_tokenize(text_unified,
...
This module implements sentence segmentation in Dutch using simple
...
def build_sentence_model(text, extra_abbrevs=None):
    ...
    # Return model instantiated with new parameters
    return PunktSentenceTokenizer(punkt_params)
>>>>>>> 25b64bf64c84e2debb8c2cf06d2a9b0eef137138
